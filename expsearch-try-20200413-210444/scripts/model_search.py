...
class Network(nn.Module):
    def __init__(self, C, num_classes, layers, criterion, steps=4, multiplier=4, stem_multiplier=3, switches_normal=[], switches_reduce=[], p=0.0,K=4):
        ...
        for i in range(layers):
            if i in [layers//3, 2*layers//3]:
                C_curr *= 2
                reduction = True
                cell = GDAS_Reduction_Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, self.p)
            else:
                reduction = False
...
#            cell = Cell(steps, multiplier, C_prev_prev, C_prev, C_curr, reduction, reduction_prev, switches)
    ...
    def forward(self, input):
        ...
        for i, cell in enumerate(self.cells):
            ...
            #     else:
            #         weights = F.softmax(self.alphas_normal, dim=-1)          #实验
            ...
            #         n += 1
            #         weights2 = torch.cat([weights2,tw2],dim=0)
            s0, s1 = s1, cell(s0, s1, self.weights,self.weights2)
    ...
    def _initialize_alphas(self):
        ...
        self.betas_normal = nn.Parameter(torch.FloatTensor(1e-3*torch.randn(k)))  #改动
